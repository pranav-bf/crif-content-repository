...
def query():
    parameters = get_time(2)
    parameters["stattype"] = "PUBLISHED"
    return {
        "query": "SELECT SUM(statcount) AS total_count, CAST(insert_date AS DATE) AS insert_day  from streamx WHERE stattype = :stattype GROUP BY insert_day ORDER BY insert_day ",
