    ...
    return [
        {
            ...
            "template": "EventSession: retrieved "
                ...
                "Windows PowerShell session row(s) for RCA. "
                "Gathered evidence for suspicious PowerShell behavior and credential access patterns."
        }, #step 1
        {
            "name": "execute_python_from_s3",
            "parameters": {
                "s3_path": "s3://automation/stream-root-cause-analysis/windows_powershell_credential_access_activity_monitoring/rca.py",
            ...
            "template": (
                "Windows PowerShell Credential Access RCA completed: "
                "{{ output.verdict if output else 'n/a' }} "
                "(score {{ output.score if output else 'n/a' }}). "
                "Heuristics executed: PowerShell-based credential access and suspicious script activity checks."
            ),
        },#step 2
        {
            "name": "evaluate",
...
{%- set process_name = (ev.get('process_name') or ev.get('image') or ev.get('process')) if ev is mapping else none -%}
{%- set process_name = process_name or ((_r0.get('process_name') or _r0.get('image') or _r0.get('process')) if _r0 is mapping else none) -%}

{%- set parent_process = (ev.get('parent_process_name') or ev.get('parent_process')) if ev is mapping else none -%}
{%- set parent_process = parent_process or ((_r0.get('parent_process_name') or _r0.get('parent_process')) if _r0 is mapping else none) -%}

{%- set command_line = (ev.get('command_line') or ev.get('process_command_line') or ev.get('cmdline')) if ev is mapping else none -%}
{%- set command_line = command_line or ((_r0.get('command_line') or _r0.get('process_command_line') or _r0.get('cmdline')) if _r0 is mapping else none) -%}
...
{% if v == 'TRUE_POSITIVE' %}
<strong class="text-danger">High-risk PowerShell credential access activity</strong>
<p>Strong evidence of credential dumping, credential theft, encoded command execution, or suspicious PowerShell activity was identified.</p>
{% elif v == 'SUSPICIOUS' %}
<strong class="text-warning">Suspicious PowerShell credential access activity</strong>
<p>Potential credential access or suspicious PowerShell behavior was observed and should be reviewed.</p>
{% elif v == 'FALSE_POSITIVE' %}
<strong>False positive</strong>
<p>The reviewed PowerShell telemetry did not meet the configured risk threshold.</p>
{% else %}
...
<div class="body-copy">
<p>Retrieved Windows PowerShell telemetry for the alert window.</p>

<span class="label">Outcome:</span>

{% if rows is not none and rows is iterable and rows is not string %}
<p>{{ rows|length }} session row(s) were reviewed for credential access behavior, encoded commands, credential dumping indicators, and suspicious PowerShell execution patterns.</p>
{% elif rows is not none %}
<p>Session output was present but returned in a non-list format.</p>
{% else %}
...
<p>
Checked PowerShell command activity,
...
<div class="evidence-row">
<div class="evidence-key">Session rows</div>
<div class="evidence-value">{{ rows|length if rows is not none and rows is iterable and rows is not string else 0 }} row(s) analysed</div>
</div>

{% if user_hint %}
<div class="evidence-row">
<div class="evidence-key">User</div>
<div class="evidence-value">{{ user_hint|e }}</div>
</div>
{% endif %}

{% if src_ip %}
<div class="evidence-row">
<div class="evidence-key">Source IP</div>
<div class="evidence-value">{{ src_ip|e }}</div>
</div>
{% endif %}

{% if host %}
<div class="evidence-row">
<div class="evidence-key">Host</div>
<div class="evidence-value">{{ host|e }}</div>
</div>
{% endif %}

{% if process_name %}
<div class="evidence-row">
<div class="evidence-key">Process</div>
<div class="evidence-value">{{ process_name|e }}</div>
</div>
{% endif %}

{% if parent_process %}
<div class="evidence-row">
<div class="evidence-key">Parent Process</div>
<div class="evidence-value">{{ parent_process|e }}</div>
</div>
{% endif %}

{% if command_line %}
<div class="evidence-row">
<div class="evidence-key">Command Line</div>
<div class="evidence-value">{{ command_line|e }}</div>
</div>
{% endif %}
...
<p class="assessment-copy">
  ...
  The RCA identified <span class="text-danger">high-risk PowerShell credential access activity or suspicious script execution behavior</span>.Investigate the affected user, host, PowerShell process, command execution history, and correlated Windows security events{% if score is not none %}. <strong>Score {{ score }}.</strong>{% endif %}
</p>

{%- elif v == 'SUSPICIOUS' -%}
<p class="assessment-copy">
  <strong>Assessment:</strong>
  <span class="text-warning">Suspicious PowerShell credential access activity</span> was observed.Validate the executed commands, process lineage, account behavior, host context, and related alerts before closing{% if score is not none %}. <strong>Score {{ score }}.</strong>{% endif %}
</p>

{%- elif v == 'FALSE_POSITIVE' -%}
<p class="assessment-copy">
  <strong>Assessment:</strong>
  No meaningful PowerShell credential access indicators met the configured RCA threshold.Treat this as a false positive unless customer context indicates otherwise{% if score is not none %}. <strong>Score {{ score }}.</strong>{% endif %}
</p>

{%- else -%}
<p class="assessment-copy">
  <strong>Assessment:</strong>
  Verdict is <strong>{{ v or 'pending or unavailable' }}</strong>{% if score is not none %}. <strong>Score {{ score }}.</strong>{% endif %}
</p>
{%- endif -%}
