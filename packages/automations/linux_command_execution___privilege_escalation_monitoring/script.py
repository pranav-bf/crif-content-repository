    ...
    return [
        {
            ...
            "template": "EventSession: retrieved "
                ...
                "Linux execution-related session row(s) for RCA. "
                "Gathered command and privilege-elevation evidence from recent host activity."
        },
        {
            "name": "execute_python_from_s3",
            "parameters": {
                "s3_path": "s3://automation/stream-root-cause-analysis/linux_command_execution_privilege_escalation_monitoring/rca.py",
            ...
            "template": (
                "Linux Command/PrivEsc RCA completed: "
                "{{ output.verdict if output else 'n/a' }} "
                "(score {{ output.score if output else 'n/a' }}). "
                "Heuristics executed: suspicious command execution and privilege escalation indicators across sampled sessions."
            ),
        },
        {
            "name": "evaluate",
