    ...
    return [
        {
            ...
            "template": "EventSession: retrieved "
                ...
                "authentication/access session row(s) for RCA. "
                "Gathered access evidence to evaluate anomalous authentication behavior and risky access patterns."
        },#step 1 
        {
            "name": "execute_python_from_s3",
            "parameters": {
                "s3_path": "s3://automation/stream-root-cause-analysis/authentication_access_security/rca.py",
            ...
            "template": (
                "Authentication Access Security RCA completed: "
                "{{ output.verdict if output else 'n/a' }} "
                "(score {{ output.score if output else 'n/a' }}). "
                "Heuristics executed: access anomaly evaluation using session context and event-level signals."
            ),
        },#step 2
        {
            "name": "evaluate",
...
def template():
    """
    RCA report for the detection UI — Fortigate Authentication & Access Security stream (HTML).
...
<li><strong>Rows returned:</strong> {{ rows|length }} auth row(s){% if src_ip or user_hint %} ({% if src_ip %}source <strong>{{ src_ip }}</strong>{% endif %}{% if src_ip and user_hint %}, {% endif %}{% if user_hint %}user <strong>{{ user_hint }}</strong>{% endif %}){% endif %}.</li>
