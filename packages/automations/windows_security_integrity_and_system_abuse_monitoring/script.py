    ...
    return [
        {
            ...
            "template": "EventSession: retrieved "
                ...
                "Windows security/integrity session row(s) for RCA. "
                "Gathered evidence for tampering, security control disablement, and system abuse indicators."
        },#step 1
        {
            "name": "execute_python_from_s3",
            "parameters": {
                "s3_path": "s3://automation/stream-root-cause-analysis/windows_security_integrity_system_abuse_monitoring/rca.py",
            ...
            "template": (
                "Windows Security Integrity/System Abuse RCA completed: "
                "{{ output.verdict if output else 'n/a' }} "
                "(score {{ output.score if output else 'n/a' }}). "
                "Heuristics executed: security control tampering and system abuse detection across sampled sessions "
                "(known_admins optional when provided)."
            ),#step 2
        },
        {
            "name": "evaluate",
